  ...
  create: (ctx: Rule.RuleContext) => {
    ...
    return {
      Program(program: Program) {
        program.body
          .filter(({ type }) =>
            ['FunctionDeclaration', 'VariableDeclaration', 'ExportNamedDeclaration', 'ExportDefaultDeclaration'].includes(type)
          )
          .map((node) => {
          ...
          .forEach((declarations: Node[]) => {
            ...
            const correctOrdering: HooksMetadata[] = [...hooks].sort(
              (a, b) => groups.indexOf(a.declaration.name) - groups.indexOf(b.declaration.name)
            );

            hooks.forEach((hook, idx) => {
              const noMatching = (): boolean =>
                correctOrdering.length > 1 && correctOrdering[idx].declaration.name !== hook.declaration.name;
              ...
              if (noMatching()) {
                ...
                const newSourceCode = format(
                  trim(source.getText()).replace(hookGoodCode, hookBadCode).replace(hookBadCode, hookGoodCode),
                  {
                    parser: 'babel',
                  }
                );
